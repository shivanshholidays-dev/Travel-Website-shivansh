...
const LoginForm = () => {
    ...
    /** After login, go to the ?redirect param or the stored redirect, else /dashboard */
    ...
    const onSubmit = async (data: LoginValues) => {
        try
        {
            ...
            const userRole = (user?.role || user?.data?.role || '').toString().toUpperCase();
        ...
        {
            toast.error(getErrorMessage(error, 'Failed to login. Please check your credentials.'));
        } finally
        {
            setIsLoading(false);
                                    ...
                                    <div className="row">
                                        <div className="col-12">
                                            <div className="togo-sign-in-heading text-center mb-30">
                                                <h4 className="togo-section-title fs-48 mb-10">Sign In</h4>
                                                <p>New user? <Link href={`/register${searchParams?.get('redirect') ? `?redirect=${searchParams.get('redirect')}` : ''}`}>Create an account.</Link></p>
                                            </div>
                                        ...
                                        <div className="col-xl-12">
                                            <div className="d-flex justify-content-between align-items-center mb-3 mt-1">
                                                <div className="togo-from-checkbox mb-0">
                                                    <input id="active" type="checkbox" />
                                                    <label htmlFor="active" className="mb-0" style={{ fontSize: '14px' }}>Remember me</label>
                                                </div>
                                                <Link href="/forgot-password" style={{ fontSize: '14px', fontWeight: 500, color: '#f26522' }}>Forgot Password?</Link>
                                            </div>

                                            <button
                                                className="togo-btn-primary w-100 d-block mb-3"
                                                type="submit"
                                                disabled={isLoading}
