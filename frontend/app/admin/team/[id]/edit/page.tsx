...
import { ArrowLeft, Upload, Facebook, Instagram, Twitter, Linkedin, Youtube } from 'lucide-react';
...
export default function AdminTeamEditPage() {
    ...
    const { useTeamMember, useUpdateTeamMember } = useAdminTeamMemberHooks();
    const { data: memberData, isLoading } = useTeamMember(memberId);
    const updateMutation = useUpdateTeamMember();

    const member = (memberData as any)?.data ?? memberData;
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try
        {
            ...
            await updateMutation.mutateAsync({ id: memberId, data: formData as any });
            ...
            <div className="togo-dashboard-booking-sec pt-50 pb-60">
                <div className="container container-1440">
                    <div className="d-flex justify-content-center align-items-center" style={{ minHeight: 300 }}>
                        <div className="text-center text-muted">Loading member data...</div>
                    </div>
                </div>
            </div>
        );
            ...
            <div className="container container-1440">
                ...
                <div className="d-flex justify-content-between align-items-center mb-40">
                    <div>
                        ...
                        <p className="text-muted small mb-0">Update <strong>{name}</strong>'s profile information.</p>
                    </div>
                    <button
                            ...
                            <div style={{ background: '#fff', borderRadius: '24px', padding: '40px', boxShadow: '0 10px 30px rgba(0,0,0,0.04)', border: '1px solid #f1f3f9' }}>
                                ...
                                <div className="row g-3 mb-3">
                                    <div className="col-md-6">
                                        ...
                                        <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="e.g. John Smith" style={inputStyle} required />
                                    </div>
                                    <div className="col-md-6">
                                        <label style={labelStyle}>Designation *</label>
                                        <input type="text" value={designation} onChange={e => setDesignation(e.target.value)} placeholder="e.g. Senior Tour Guide" style={inputStyle} required />
                                    </div>
                                </div>
                                ...
                                {/* Social Links */}
                                <div className="mt-30">
                                    <h6 style={{ fontWeight: 800, fontSize: '15px', marginBottom: '20px', color: '#333' }}>Social Media Links</h6>
                                    <div className="row g-3">
                                        {[
                                            ...
                                            <div className="col-md-6" key={key}>
                                                ...
                                                <input
                                                    type="url" value={(socialLinks as any)[key]}
                        ...
                        <div className="col-lg-4">
                            <div style={{ background: '#fff', borderRadius: '24px', padding: '30px', boxShadow: '0 10px 30px rgba(0,0,0,0.04)', border: '1px solid #f1f3f9', position: 'sticky', top: '20px' }}>
                                    ...
                                    <div style={{ border: '2px dashed #ddd', borderRadius: '16px', padding: '20px', textAlign: 'center', background: '#fafafa' }}>
                                        {imagePreview && (
                                            <img src={imagePreview} alt="Preview"
                                                style={{ width: 80, height: 80, borderRadius: '50%', objectFit: 'cover', marginBottom: 12, border: '3px solid #eee' }}
                                            />
                                        )}
                                        <input type="file" id="edit-member-image" accept="image/*" className="d-none" onChange={handleImageChange} />
                                        <label htmlFor="edit-member-image" style={{ cursor: 'pointer', margin: 0 }}>
                                            <div className="d-flex flex-column align-items-center" style={{ gap: 6 }}>
                                                ...
                                                <span style={{ fontWeight: 600, fontSize: 12, color: '#333' }}>
                                                    {imageFile ? imageFile.name : imagePreview ? 'Change photo' : 'Upload photo'}
                                                </span>
                                ...
                                <div className="mb-25">
                                    ...
                                    <p style={{ fontSize: 11, color: '#999', marginTop: 6, marginBottom: 0 }}>Lower number = shown first</p>
                                </div>
